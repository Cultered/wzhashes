 ...
 * Uses Ed25519 for key generation and SHA256 for hashing
 * 
...
// ============================================================
// SHA256 Implementation (optimized)
// ============================================================
...
public:
    ...
    void finalize(uint8_t* hash) {
        ...
        m_bitlen += m_buflen * 8;
        m_buffer[63] = m_bitlen;
        m_buffer[62] = m_bitlen >> 8;
        m_buffer[61] = m_bitlen >> 16;
        m_buffer[60] = m_bitlen >> 24;
        m_buffer[59] = m_bitlen >> 32;
        m_buffer[58] = m_bitlen >> 40;
        m_buffer[57] = m_bitlen >> 48;
        m_buffer[56] = m_bitlen >> 56;
        transform();
...
void worker(int id, const std::string& pattern) {
    ...
    uint8_t seed[32], public_key[32], hash[32];
    ...
    while (!g_found.load(std::memory_order_relaxed)) {
        for (int i = 0; i < batch_size && !g_found.load(std::memory_order_relaxed); i++) {
            ...
            SHA256::hash(public_key, 32, hash);
            
            std::string hash_hex = to_hex(hash, 32);
            local_attempts++;
            
            if (std::regex_search(hash_hex, re)) {
...
int main(int argc, char* argv[]) {
    std::cout << "\n============================================================\n";
    std::cout << "PARALLEL BRUTEFORCE VANITY KEY GENERATOR (C++ Ed25519)\n";
    std::cout << "============================================================\n";
    ...
    if (result) {
        ...
        std::cout << "Public Key (base64):  " << to_base64(result->public_key, 32) << "\n";
        std::cout << "============================================================\n";
